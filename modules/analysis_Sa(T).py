...
def compute_pseudo_spectral_acceleration(acc_g: np.ndarray, dt: float, period: float, damping_ratio: float = SA_SCALING_DAMPING) -> float:
    acc_g = np.asarray(acc_g, dtype=float)
    if acc_g.ndim != 1 or acc_g.size < 2:
        raise ValueError("Ground motion acceleration array must be one-dimensional with at least two samples.")
    if dt <= 0.0:
        raise ValueError("Time step dt must be positive.")
    if period <= 0.0:
        raise ValueError("Sa(T) period must be positive.")
    ...
    npts = acc_g.size
    disp = np.zeros(npts, dtype=float)
...
def analyze(
    ...
    if base_period is None or float(base_period) <= 0.0:
        raise ValueError("A positive base period T is required for the Sa(T) workflow.")
    ...
    for record in list_gmrs_data:
        try:
            ...
            if record_sa_t <= MIN_IM_TOL:
                raise ValueError(f"Ground motion {record} has near-zero Sa(T={base_period:.4g}s); cannot scale.")

            for scale in np.arange(min_scale, max_scale + increment, increment):
                target_sa = round(float(scale), 8)
                scale_factor = 0.0 if np.isclose(target_sa, 0.0) else target_sa / record_sa_t
                ...
                if fast_mode and state == "not intersected":
                    remaining_targets = np.arange(target_sa + increment, max_scale + increment, increment)
                    for remaining in remaining_targets:
                        remaining = round(float(remaining), 8)
                        sdv_point.append(fail_state_displ)
                        im_point.append(remaining)
                        ...
                        if progress_callback:
                            progress_callback()
                    logger.info(
                        "[FAST-MODE] %s: first miss at target Sa(T)=%.6f g; skipped %d higher levels.",
                        record,
                        target_sa,
                ...
                logger.info(
                    "Record: %s, Target Sa(T): %.6f g, Scale factor: %.6f, State: %s",
                    record,
                    target_sa,
                    scale_factor,
                    state,
